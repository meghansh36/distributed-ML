...
class Packet:
    ...
    def pack(self) -> bytes:
        ...
        return struct.pack(f"i{255}s{6}si{2048 * 8}s", len(self.sender), self.sender.encode('utf-8'), self.type.encode('utf-8'), len(jsondata), jsondata.encode())
    ...
    def unpack(recvPacket: bytes):
        ...
        try:
            unpacked_tuple: tuple[bytearray] = struct.unpack(
                f"i{255}s{6}si{2048 * 8}s", recvPacket)
